...
In regards of the Random Forest, two tests were conducted, that is, Random Forest with no max d with max depth 3 epth 3. The results show that R-squared  dropped from 100% to 73.70% when limiting the maximum depth to 3. But Random Forest model still demonstrates a significant improvement compared to the Logistic Regression model. As it was also attested by the results from its confusion matrix as described delow:
...
- True Positive 5102 out of 6950 predictions, which corresponds to 73%
- False Negative: 1848 out of 6950 predictions, which corresponds to 27%

While for the class FulfilmentB: Merchant = 0.
- True se Negative 397 out 556 predictions, which accounts for 71%
- False Positive: 159 out of 556 predictions, which accounts for 29%.
